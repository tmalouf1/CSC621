	...
	if(!empty($firstName) || !empty($lastName) || !empty($email) || !empty($hsGradYear) || !empty($height) 
		...
		if(mysqli_connect_error()){
			...
			if($rnum == 0){
				...
				$select = "SELECT idRecruit FROM Recruits WHERE email = ?";

				$insert = "INSERT INTO Player_Attributes (Recruits_idRecruit,height,weight,experience,position) VALUES (?,?,?,?,?)";
				...
				$stmt = $conn -> prepare($insert);
				$stmt -> bind_param("iiiis", $assoc['idRecruit'],$height,$weight,$experience,$position);
				$stmt -> execute();
